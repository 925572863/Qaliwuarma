...
@section('breadcrumb', 'Distribución de PECOSA — Nivel Inicial')
...
<div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
    ...
    {{-- Cabecera --}}
    <div class="p-5 border-b border-gray-100 flex justify-between items-center bg-gradient-to-r from-orange-50 to-transparent no-print">
        <div>
            <h2 class="text-lg font-bold text-gray-800">Distribución de Productos por Sección</h2>
            <p class="text-xs text-gray-500 mt-0.5">
                {{ $totalAlumnos }} alumnos activos ·
                @if($hayGuardado)
                    <span class="text-orange-600 font-medium">Guardado el {{ \Carbon\Carbon::parse($ultimaActualizacion)->format('d/m/Y H:i') }}</span>
                @else
    ...
    {{-- Formulario --}}
    <form method="POST" action="{{ route('pecosa.inicial.prorrateo.guardar') }}" id="form-distribucion">
        @csrf
                ...
                <tbody>
                    @foreach($data as $fila)
                        <tr class="hover:bg-orange-50 transition-colors group">
                            {{-- Celda sección --}}
                            <td class="px-3 py-2 border border-gray-200 font-bold text-gray-700 text-center sticky left-0 z-10 bg-white group-hover:bg-orange-50 text-xs fila-alumnos"
        ...
        <div class="p-5 border-t border-gray-100 no-print">
            <div class="flex flex-col sm:flex-row sm:items-center gap-3">
                ...
                <div class="flex gap-2">
                    ...
                    <button type="submit"
                            class="px-5 py-2.5 bg-orange-600 hover:bg-orange-700 text-white rounded-lg text-sm font-semibold transition-colors flex items-center space-x-2 shadow-sm">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
function distribuirAutomatico() {
    ...
    document.querySelectorAll('.cantidad-restante').forEach(td => {
        ...
        // Método del resto mayor: reparte totalProducto proporcional a los
        // alumnos de cada fila, garantizando que la suma sea exacta.
...
<style>
    @media print {
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        th, td { padding: 2px 1px !important; border: 0.5px solid #666 !important; word-break: break-word !important; overflow: hidden !important; }
        .sticky { position: static !important; }
        input[type=number] { border: none !important; background: transparent !important; text-align: center !important; width: 100% !important; font-size: 6.5px !important; padding: 0 !important; -moz-appearance: textfield; }
