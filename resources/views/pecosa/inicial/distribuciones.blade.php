...
@section('breadcrumb', 'Distribuciones guardadas — Nivel Inicial')
...
<div id="modalImportar" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/40">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-md mx-4 p-6">
        ...
        <form method="POST" action="{{ route('pecosa.inicial.distribuciones.importar') }}" enctype="multipart/form-data">
            @csrf
            <div class="mb-3">
            ...
            <div class="mb-5">
                ...
                <input type="file" name="archivo" accept=".xlsx,.xls" required
                       class="w-full text-sm text-gray-600 file:mr-3 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-medium file:bg-orange-50 file:text-orange-700 hover:file:bg-orange-100">
            </div>
...
<div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
    ...
    <div class="p-5 border-b border-gray-100 flex justify-between items-center bg-gradient-to-r from-orange-50 to-transparent">
        <div>
        ...
        <div class="flex items-center space-x-2">
            <button onclick="document.getElementById('modalImportar').classList.remove('hidden')"
                    class="px-4 py-2 bg-white border border-orange-500 text-orange-700 hover:bg-orange-50 rounded-lg text-sm font-medium transition-colors flex items-center space-x-2">
            ...
            <a href="{{ route('pecosa.inicial.prorrateo') }}"
               class="px-4 py-2 bg-orange-600 hover:bg-orange-700 text-white rounded-lg text-sm font-medium transition-colors flex items-center space-x-2">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    @if($versiones->isEmpty())
        <div class="py-20 text-center">
            ...
            <a href="{{ route('pecosa.inicial.prorrateo') }}"
               class="mt-3 inline-block bg-orange-600 hover:bg-orange-700 text-white text-sm font-medium px-4 py-2 rounded-lg transition-colors">
        ...
        <div class="divide-y divide-gray-100">
            @foreach($versiones as $version)
                <div class="flex items-center justify-between px-6 py-4 hover:bg-gray-50 transition-colors">
                    ...
                    <div class="flex items-center space-x-2">
                        <a href="{{ route('pecosa.inicial.distribuciones.ver', $version->id) }}"
                        ...
                        <form method="POST"
                              action="{{ route('pecosa.inicial.distribuciones.eliminar', $version->id) }}"
