@extends('layouts.app')
@section('title', 'Registrar PECOSA — Inicial')
@section('page-title', 'Registrar Productos PECOSA')
@section('breadcrumb', 'Pecosa Inicial › Nuevo registro')
...
<form method="POST" action="{{ route('pecosa.inicial.store') }}" id="form-pecosa">
@csrf

<div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">

    {{-- Cabecera estilo PECOSA --}}
    <div class="bg-yellow-400 px-6 py-3 flex items-center justify-between">
        <div>
            <p class="text-xs font-black text-yellow-900 uppercase tracking-widest">Pecosa — Nivel Inicial</p>
            <p class="text-[10px] text-yellow-800 mt-0.5">Rellena fila por fila igual que el documento físico</p>
        </div>
        <div class="flex items-center gap-2">
            <button type="button" onclick="agregarFila()"
                    class="px-3 py-1.5 bg-yellow-600 hover:bg-yellow-700 text-white rounded-lg text-xs font-bold flex items-center gap-1.5 transition-colors">
        ...
        <table class="w-full text-sm border-collapse" id="tabla-pecosa">
            <thead>
                <tr class="bg-gray-800 text-white text-[10px] uppercase tracking-wide">
                    ...
                    <th class="border border-gray-600 px-3 py-2.5 text-center w-24 bg-yellow-700">VOLUMEN</th>
                    ...
                    <th class="border border-gray-600 px-2 py-2.5 text-center w-10 no-print"></th>
                </tr>
            </thead>
    ...
    <div class="px-6 py-4 border-t border-gray-100 flex items-center justify-between bg-gray-50">
        ...
        <div class="flex items-center gap-3">
            ...
            <button type="submit"
                    class="px-6 py-2 bg-yellow-500 hover:bg-yellow-600 text-white rounded-lg text-sm font-bold shadow-sm transition-colors flex items-center gap-2">
...
function agregarFila(data = {}) {
    ...
    tr.innerHTML = `
        ...
        <td class="border border-gray-200 px-1 py-1">
            <input type="number" name="filas[${i}][cant]" value="${data.cant ?? ''}"
                   ...
                   class="w-full px-2 py-1.5 border-0 text-center text-sm font-bold focus:outline-none focus:ring-2 focus:ring-yellow-400 rounded bg-transparent"
                   placeholder="0">
        </td>
        <td class="border border-gray-200 px-1 py-1">
            <select name="filas[${i}][unid]" required
                    class="w-full px-2 py-1.5 border-0 text-center text-sm focus:outline-none focus:ring-2 focus:ring-yellow-400 rounded bg-transparent">
                <option value="">—</option>
        ...
        <td class="border border-gray-200 px-1 py-1">
            <input type="text" name="filas[${i}][descripcion]" value="${data.descripcion ?? ''}"
                   ...
                   class="w-full px-2 py-1.5 border-0 text-sm uppercase focus:outline-none focus:ring-2 focus:ring-yellow-400 rounded bg-transparent"
        ...
        <td class="border border-gray-200 px-1 py-1 bg-yellow-50">
            <input type="text" id="vol-${i}" readonly
                   class="w-full px-2 py-1.5 border-0 text-center text-sm font-bold text-yellow-700 bg-transparent focus:outline-none"
                   placeholder="—">
        </td>
        <td class="border border-gray-200 px-1 py-1">
            <input type="text" name="filas[${i}][lote]" value="${data.lote ?? ''}"
                   maxlength="200"
                   class="w-full px-2 py-1.5 border-0 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-400 rounded bg-transparent"
                   placeholder="LT.00-0">
        </td>
        <td class="border border-gray-200 px-1 py-1">
    ...
    // Enfocar el campo CANT de la nueva fila
...
// Tab en última celda de una fila → nueva fila automáticamente
