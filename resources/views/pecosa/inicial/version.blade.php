...
<div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
    ...
    <div class="p-5 border-t border-gray-100 no-print">
        ...
        <div class="flex flex-wrap gap-2">
            @foreach($data as $fila)
                <a href="{{ route('pecosa.inicial.distribuciones.listado', ['version' => $version->id, 'seccion' => $fila['seccion']]) }}"
                   class="px-3 py-1.5 bg-orange-400 hover:bg-orange-500 text-white rounded-lg text-xs font-bold transition-colors">
                    {{ $fila['seccion'] }}
                    <span class="text-orange-100 font-normal">({{ $fila['alumnos'] }})</span>
                </a>
            @endforeach
        </div>
    </div>
...
function descargarExcel() {
    ...
    ws['!cols'] = [{ wch: 16 }, { wch: 9 }, ...nombres.map(() => ({ wch: 22 })), { wch: 10 }];

    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Distribución');
    XLSX.writeFile(wb, 'Distribucion_Inicial_' + fecha + '.xlsx');
}
</script>
