...
@section('header-actions')
    <div class="flex items-center gap-2">
        ...
        {{-- Subir Foto (IA) --}}
        <button type="button" onclick="document.getElementById('modal-foto-inicial').classList.remove('hidden')"
...
<a href="{{ route('pecosa.inicial.create') }}"
        ...
        {{-- Eliminar todo --}}
        <form method="POST" action="{{ route('pecosa.inicial.destroy-all') }}"
              onsubmit="return confirm('¿Eliminar TODOS los productos de Pecosa Inicial? Esta acción no se puede deshacer.')">
            @csrf @method('DELETE')
            <button type="submit"
    ...
    <div id="modal-excel-inicial" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/50">
        <div class="bg-white rounded-2xl shadow-xl w-full max-w-md mx-4 p-6">
            ...
            <p class="text-xs text-gray-500 mb-2">El archivo debe tener las columnas en este orden:<br>
                <span class="font-mono text-gray-700">CANT | UNID | DESCRIPCIÓN | MARCA | PRESENTACIÓN | LOTE</span>
            </p>
            <a href="{{ route('pecosa.inicial.plantilla') }}" class="inline-flex items-center text-xs text-green-700 hover:text-green-800 font-medium mb-4">
    ...
    {{-- Modal subir foto (IA) --}}
    <div id="modal-foto-inicial" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/50">
        <div class="bg-white rounded-2xl shadow-xl w-full max-w-md mx-4 p-6">
            ...
            <form method="POST" action="{{ route('pecosa.inicial.importar-foto') }}" enctype="multipart/form-data">
                @csrf
                <div class="mb-4">
                    <label class="block text-xs font-semibold text-gray-600 mb-1">Nombre de esta Pecosa (opcional)</label>
                    <input type="text" name="nombre_pecosa" placeholder="Ej: Pecosa N° 324426"
                ...
                <div class="flex justify-end gap-3">
                    <button type="button" onclick="document.getElementById('modal-foto-inicial').classList.add('hidden')"
                            class="px-4 py-2 border border-gray-300 text-gray-600 rounded-lg text-sm hover:bg-gray-50">
                        Cancelar
                    </button>
                    <button type="submit"
            ...
            <table class="w-full text-sm border-collapse">
                <thead>
                    <tr class="bg-yellow-500 text-white text-xs uppercase tracking-wider">
                        ...
                        <th class="px-4 py-3 text-left border border-yellow-400">PECOSA</th>
