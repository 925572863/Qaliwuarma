...
@section('header-actions')
    <div class="flex items-center gap-2">
        ...
        {{-- Eliminar todo --}}
        <form method="POST" action="{{ route('pecosa.primaria.destroy-all') }}"
              onsubmit="return confirm('¿Eliminar TODOS los productos de Pecosa Primaria? Esta acción no se puede deshacer.')">
            @csrf @method('DELETE')
            <button type="submit"
...
@if($viendoSoloReciente)
    <div class="bg-blue-50 border border-blue-200 text-blue-800 text-sm rounded-xl px-4 py-3 mb-5 flex items-center justify-between">
        ...
        <a href="{{ route('pecosa.primaria.index', ['todas' => 1]) }}" class="text-blue-700 font-semibold hover:underline whitespace-nowrap ml-4">Ver historial completo →</a>
    </div>
@endif
